...
const songInsertSchema = createInsertSchema(schema.song).omit("uploadDate");
const chartInsertSchema = createInsertSchema(schema.chart).omit("id");
const chartHitObjectInsertSchema = createInsertSchema(schema.chartHitObject).omit("id");
	...
	.handler(async ({ input, context }) => {
		...
		try {
			await db.transaction(async (tx) => {
				const newSong = await tx.insert(schema.song).values(validatedSongData).returning({ id: schema.song.id });
				const newSongId = newSong[0]?.id;
				if (!newSongId) {
					throw new Error('Failed to insert song');
				}

				for (const [i, chartData] of processedData.charts.entries()) {
					...
					const newChart = await tx.insert(schema.chart).values(validatedChartData).returning({ id: schema.chart.id });
					const newChartId = newChart[0]?.id;
					if (!newChartId) {
						throw new Error('Failed to insert chart');
					}
					...
					if (hitObjectsForChart && Array.isArray(hitObjectsForChart) && hitObjectsForChart.length > 0) {
						...
						await tx.insert(schema.chartHitObject).values(hitObjectInserts);
					}
				}
			});
