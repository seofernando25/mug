import { HIT_ZONE_CENTER, LANE_BACKGROUNDS, LANE_COLORS } from '$lib/colors';
...
type GameplayNote = typeof schema.chartHitObject.$inferSelect & {
...
export async function createGame(
    ...
    function _renderLoopContent(currentTimeMs: number) {
        ...
        const activeNotesForUpdate: Array<typeof schema.chartHitObject.$inferSelect & { isActivelyHeld?: boolean }> = [];
        ...
        for (const gameNote of notePool.getActiveNotes()) {
            ...
            if (gameplayNote && gameplayNote.note_type === 'hold') {
                ...
                if (gameplayNote.holdBroken) {
                    const brokenColor = LANE_BACKGROUNDS[gameplayNote.lane % LANE_BACKGROUNDS.length];
                    ...
                    const activeColor = LANE_COLORS[gameplayNote.lane % LANE_COLORS.length];
                    ...
                    const defaultLaneColor = LANE_COLORS[gameplayNote.lane % LANE_COLORS.length];
                ...
                gameNote.headGraphics.tint = LANE_COLORS[gameplayNote.lane % LANE_COLORS.length];
            }
        }
    ...
    function _processNoteHit(key: string, laneIndex: number) {
        ...
        for (let i = upcomingNoteIndex; i < notes.length; i++) {
            ...
            if (absTimeDifference <= MEH_WINDOW_MS) {
                ...
                callbacks.onScoreUpdate(currentScore, currentCombo, maxCombo);
                callbacks.onNoteHit(note, judgment, LANE_COLORS[laneIndex]);
            ...
            if (lane !== -1) {
                ...
                if (activeHoldNote) {
                    ...
                    if (legitimateRelease) {
                        ...
                        callbacks.onNoteHit(activeHoldNote, judgment, LANE_COLORS[lane]);
