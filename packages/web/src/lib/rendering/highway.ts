import { LANE_COLORS, LANE_BACKGROUNDS, LANE_BACKGROUND_ALPHA, HIGHWAY_LINE } from '$lib/colors';
import { HIGHWAY_LINE_THICKNESS } from './constants';
...
export function drawHighway(
	...
	for (let i = 0; i < metricsSnapshot.numLanes; i++) {
		const illumination = new LaneIllumination(
			...
			LANE_COLORS[i % LANE_COLORS.length] // Use lane background color
		);
	...
	function _drawInternal() {
		...
		for (let i = 0; i < metrics.numLanes; i++) {
			...
			mainRectsGraphics.rect(i * metrics.laneWidth, 0, metrics.laneWidth, metrics.height)
				.fill({ color: LANE_BACKGROUNDS[i % LANE_BACKGROUNDS.length], alpha: LANE_BACKGROUND_ALPHA });
		}
		lineGraphics.clear();
		for (let i = 0; i < metrics.numLanes + 1; i++) {
			// Draw lines relative to highwayContainer
			const xPos = i * metrics.laneWidth;
			lineGraphics.rect(xPos - HIGHWAY_LINE_THICKNESS / 2, 0, HIGHWAY_LINE_THICKNESS, metrics.height)
				.fill({ color: HIGHWAY_LINE });
		}
	}
...
export function drawHighwayLines(
	...
	for (let i = 0; i < lanes + 1; i++) {
		...
		lineGraphics.rect(xPos - HIGHWAY_LINE_THICKNESS / 2, 0, HIGHWAY_LINE_THICKNESS, stageHeight)
			.fill({ color: HIGHWAY_LINE });
	}
} 