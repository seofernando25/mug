...
export class NotePool {
	...
	private _prepopulatePools(size: number) {
		for (let i = 0; i < size; i++) {
			this.tapNotePool.push(new GameNote({ id: -1, time: 0, lane: 0, note_type: 'tap', duration: null, chartId: '' },
				this.currentLaneWidth));
			this.holdNotePool.push(new HoldNote({ id: -1, time: 0, lane: 0, note_type: 'hold', duration: 100, chartId: '' },
				this.currentLaneWidth));
		}
	}

	getNote(noteData: typeof schema.chartHitObject.$inferSelect): GameNote {
		let note: GameNote;
