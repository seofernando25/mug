...
function parseOsuFileContent(content: string): ParsedOsuData {
	...
	return { metadata, timingPoints, hitObjects, sections };
}

function convertOsuDataToChart(osuData: ParsedOsuData): {
	...
	const calculatedBpm = bpm ? Math.round(60000 / bpm) : 120;
...
export async function processFileAndExtractData(
	...
	if (fileExtension === "osz") {
		...
		if (osuDataWithEvents) {
			...
			if (bgEvent) {
				...
				if (parts.length > 2) {
					let bgFilename = parts[2].replace(/"/g, "");
		...
		const previewTime = primaryOsuData.metadata["PreviewTime"]
			? Number(primaryOsuData.metadata["PreviewTime"])
			: 0;
		...
		if (
			!mugFileData.metadata?.title ||
			!mugFileData.metadata?.artist ||
	...
	if (
		...
		!mugData.metadata ||
		!mugData.charts ||
		!Array.isArray(mugData.charts) ||
		mugData.charts.length === 0 ||
		!mugData.hitObjects ||
		!Array.isArray(mugData.hitObjects) ||
		mugData.hitObjects.length !== mugData.charts.length ||
		!audioContent ||
