<script lang="ts">
	...
	const menuItems = [
		{ label: 'SOLO', href: '/solo', color: 'text-purple-400', desc: 'Play standard rhythm game' },
		{
			...
			href: '/multiplayer',
			color: 'text-cyan-400',
			desc: 'Compete with others online'
		},
		{
			label: 'EDITOR',
			href: '/level-creator',
			color: 'text-yellow-400',
			desc: 'Create your own charts'
		},
	...
	<div class="relative z-10 flex w-full h-full pb-24">
		...
		<div class="w-1/2 flex flex-col justify-center pl-24" in:fly={{ x: -50, duration: 800 }}>
			<h1 class="text-9xl font-black italic tracking-tighter text-white drop-shadow-2xl">
				MUG
				<span class="text-transparent bg-clip-text bg-gradient-to-br from-purple-400 to-cyan-400"
					>.</span
				>
			</h1>
			...
			<div
				class="mt-12 h-16 transition-opacity duration-300 {hoveredIndex !== null
					? 'opacity-100'
					: 'opacity-0'}"
			>
