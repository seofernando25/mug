<script lang="ts">
	...
	onMount(() => {
		...
		if (!roomState || !roomState.currentChart?.songId) {
			console.warn(
				'[Multiplayer Game] No room state or chart data found, redirecting to room lobby'
			);
...
{#if isHoldingEsc && !isLoading && !error}
	...
	>
		...
		<div class="w-full text-center pb-4 bg-gradient-to-t from-gray-900/90 to-transparent">
			<p
				class="text-red-500 font-black tracking-[0.3em] text-xl uppercase drop-shadow-[0_2px_10px_rgba(220,38,38,0.5)] animate-pulse"
			>
				Hold to Forfeit
			</p>
		</div>
