...
<div
	class="bg-gray-900/80 border-b border-white/10 backdrop-blur-md p-4 flex items-center justify-between shadow-lg z-20 relative"
>
	<!-- Left: Room Info -->
		...
		<div class="flex flex-col">
			<div class="flex items-center gap-3">
				...
				<div class="flex gap-1">
					<span
						class="text-[10px] font-bold bg-purple-500/20 text-purple-300 px-2 py-0.5 rounded border border-purple-500/30 uppercase tracking-wider"
						>Versus</span
					>
				</div>
			</div>
			...
			<div class="flex items-center gap-2 mt-1">
				...
				<span class="font-mono text-xs text-gray-400 bg-black/30 px-1.5 py-0.5 rounded select-all"
					>{roomId}</span
				>
			</div>
		</div>
	...
	<div class="relative">
		{#if isHost}
			<button
				...
				class="flex items-center gap-2 px-4 py-2 bg-purple-600/20 hover:bg-purple-600/40 border border-purple-500/50 rounded text-xs font-bold text-purple-300 uppercase tracking-wider transition-colors {isSettingsOpen
					? 'bg-purple-600/40'
					: ''}"
			>
					...
					<div class="space-y-4">
						...
						<div>
							<label
								for="roomPass"
								class="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-1"
								>Password <span class="text-gray-600 font-normal normal-case">(Optional)</span
								></label
							>
							<input
								type="password"
