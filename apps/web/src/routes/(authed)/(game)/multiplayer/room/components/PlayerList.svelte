...
<div class="h-full flex flex-col gap-2 p-4 overflow-y-auto">
	...
	{#each players as player (player.userId)}
		<div
			class="group relative bg-gray-800 border-l-4 border-transparent p-2 rounded flex items-center gap-3 transition-colors hover:bg-gray-750
		...
		>
			<div class="relative">
				...
				{:else}
					<div
						class="w-8 h-8 rounded bg-gray-700 flex items-center justify-center text-[10px] font-bold text-gray-400"
					>
						?
					</div>
				{/if}
			...
			<div class="flex flex-col min-w-0">
				<span
					class="font-bold text-xs text-white truncate group-hover:text-purple-300 transition-colors"
				>
			...
			<div class="ml-auto">
				...
				<div
					class="px-2 py-0.5 rounded bg-green-500/20 text-green-400 text-[9px] font-bold border border-green-500/30 uppercase"
				>
					Ready
				</div>
			</div>
