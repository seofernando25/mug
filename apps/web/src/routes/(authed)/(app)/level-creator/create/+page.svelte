...
<div class="flex flex-col h-[calc(100vh-4rem)] bg-gray-950 text-gray-100 overflow-hidden">
	<div
		class="h-16 bg-gray-900 border-b border-white/10 flex items-center px-6 justify-between shrink-0 z-20"
	>
			...
			<div class="absolute inset-0 flex items-center justify-center z-10 p-8">
				<div
                    ...
                    {isMusicDragging
						...
						: 'hover:border-purple-500/30 hover:bg-gray-800/80'}"
					role="button"
					tabindex="0"
				...
				>
					<div
						class="w-20 h-20 bg-gray-800 rounded-full flex items-center justify-center mb-6 shadow-xl border border-white/5"
					>
						<svg
						...
						>
							<path
								...
								d="M9 9l3-3m0 0l3 3m-3-3v12a11.25 11.25 0 01-5.982-2.275M15 9.75a3 3 0 11-6 0 3 3 0 016 0z"
							/>
						</svg>
					</div>
					...
					<button
						class="bg-purple-600 hover:bg-purple-500 text-white font-bold py-3 px-8 rounded-full transition-all shadow-lg shadow-purple-900/20"
					>
						Select File
					</button>
					...
					{#if musicErrorMessage}
						<p
							class="text-red-400 mt-6 text-sm bg-red-900/20 px-4 py-2 rounded-lg border border-red-500/20"
						>
							{musicErrorMessage}
						</p>
					{/if}
			...
			<div class="flex-1 flex flex-col relative bg-gray-950">
				<div class="flex-grow relative flex justify-center bg-black/20">
					{#if musicFileReady && audioUrl && editorState}
						<LevelEditor {editorState} {audioUrl} />
					{/if}
				</div>

				<div
					class="h-20 bg-gray-900 border-t border-white/10 shrink-0 flex items-center px-6 gap-4 z-20"
				>
					<audio
						src={audioUrl}
						controls
						class="w-full h-10 opacity-70 hover:opacity-100 transition-opacity invert hue-rotate-180"
					></audio>
				</div>
			</div>
		{/if}
