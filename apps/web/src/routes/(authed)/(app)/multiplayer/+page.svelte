<script lang="ts">
	...
	async function handleCreateRoomSubmit() {
		...
		try {
			...
			await gameSocket.waitForPacket(
				(pkt) => (pkt?.op === 'ack' && pkt.data?.message === 'room_created') || null
			);

			const roomState = await gameSocket.waitForPacket(
				(pkt) =>
					pkt?.op === 'room_state' && pkt.data?.name === newRoomName.trim() ? pkt.data : null,
				3000
			);
			...
			createRoomError =
				e instanceof Error ? e.message : 'An exception occurred while creating the room.';
		}
...
<div class="flex flex-col">
	<!-- HEADER -->
	<header
		class="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 relative z-40"
	>
		<!-- Left: Create Panel Container -->
		<div class="flex flex-col relative">
			<h1 class="text-4xl font-black italic tracking-tighter text-white drop-shadow-md mb-2">
				MULTIPLAYER <span
					class="text-transparent bg-clip-text bg-gradient-to-br from-purple-400 to-cyan-400"
					>LOUNGE</span
				>
			</h1>
					...
					>
						...
						<div>
							<label
								for="roomPassword"
								class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1"
								>Password <span class="text-gray-600 normal-case font-normal">(Optional)</span
								></label
							>
							<input
								type="password"
						...
						<div>
							<span class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1"
								>Beatmap</span
							>
							<button
								type="button"
							...
							>
								{#if selectedSong}
									<!-- Selected State -->
									<div
										class="absolute inset-0 bg-cover bg-center opacity-60 group-hover:opacity-40 transition-opacity"
										style:background-image="url({selectedSong.imageUrl})"
									></div>
									<div
									...
									<div
										class="absolute right-3 top-1/2 -translate-y-1/2 z-10 text-xs bg-gray-800/80 px-2 py-1 rounded text-gray-300 border border-gray-600"
									>
										CHANGE
									</div>
								{:else}
									<!-- Empty State -->
									<span
										class="text-sm font-bold text-gray-500 group-hover:text-purple-400 transition-colors"
										>Select a Beatmap</span
									>
								{/if}
							</button>
						</div>
						...
						{#if createRoomError}
							<div
								class="p-2 rounded bg-red-900/30 border border-red-500/30 text-red-300 text-xs font-medium"
							>
								{createRoomError}
							</div>
						{/if}
			...
			{#if showFilterDropdown}
				...
				>
					<div class="space-y-4">
						...
						<!-- Room Type -->
						<div>
							<span class="block text-sm font-bold text-gray-400 uppercase tracking-wider mb-2"
								>Room Type</span
							>
	...
	{#if status === 'connecting' || isLoading}
		<div class="flex-1 flex items-center justify-center">
			<div class="animate-pulse flex flex-col items-center">
				<div
					class="h-12 w-12 border-4 border-purple-500 border-t-transparent rounded-full animate-spin mb-4"
				></div>
	...
	{:else if error}
		<div
			class="bg-red-900/50 border border-red-500 text-red-200 px-6 py-4 rounded-xl my-4 text-center backdrop-blur-sm"
		>
