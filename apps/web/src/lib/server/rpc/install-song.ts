...
import { uploadJobSchema } from '@mug/contract';
	...
	.handler(async ({ input, context }) => {
		...
		if (USE_PROCESSOR) {
			...
			try {
				...
				const jobResult = uploadJobSchema({
					jobId,
					userId: uploaderId,
					s3Key,
				});
				if (jobResult instanceof type.errors) {
					throw new Error(jobResult.toString());
				}
				const job: UploadJob = jobResult;
