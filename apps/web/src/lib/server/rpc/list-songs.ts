...
export const listSongsProcedure = routerBaseContext
	...
	.handler(async ({ input }) => {
		try {
			...
			const items = (Array.isArray(songsWithCharts) ? songsWithCharts : []).map((s) => {
				const charts = Array.isArray(s.charts) ? s.charts : [];
				const difficultyNames = charts.map((c) => c.difficultyName);
			...
			// Sign the image urls
			const itemsWithSignedUrls = await Promise.all(items.map(async (item: typeof items[0]) => {
				if (item.imageS3Key) {
