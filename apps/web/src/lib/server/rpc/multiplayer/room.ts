	...
	.handler(async ({ input, context }) => {
		...
		try {
			...
			if (newRoom.currentChartId) {
				...
				if (chartData && chartData.song && typeof chartData.song === 'object' && 'title' in chartData.song) {
					const song = chartData.song as { title: string; artist: string; imageS3Key: string | null };
					chartDetails = {
						name: song.title,
						artist: song.artist,
...
export const getRoomProcedure = routerBaseContext
	...
	.handler(async ({ input, context }) => {
		...
		let chartDetails = null;
		if (roomData.currentChart && roomData.currentChart.song && typeof roomData.currentChart.song === 'object' && 'title' in roomData.currentChart.song) {
			const song = roomData.currentChart.song as { title: string; artist: string; imageS3Key: string | null };
			chartDetails = {
