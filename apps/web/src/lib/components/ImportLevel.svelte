...
<div
	class="group relative flex flex-col justify-between h-64 p-8 rounded-2xl bg-gray-900/60 backdrop-blur-md border border-white/10 transition-all duration-300 hover:bg-gray-800/80 hover:border-blue-500/50 hover:shadow-2xl hover:shadow-blue-500/10 overflow-hidden"
>
	<div
		class="absolute top-0 right-0 p-32 bg-blue-600/10 rounded-full blur-3xl -mr-16 -mt-16 transition-all group-hover:bg-blue-600/20"
	></div>

	<div class="relative z-10 pointer-events-none">
		<div
			class="w-14 h-14 mb-6 rounded-xl bg-blue-500/20 flex items-center justify-center text-blue-300 group-hover:text-white group-hover:bg-blue-500 transition-colors"
		>
			<svg
	...
	<div class="relative z-10 grow flex flex-col justify-end">
		...
		<button
            ...
            {isDragging
				...
				: 'hover:border-blue-400/50 hover:bg-white/5'}
		...
		>
			{#if !importedFile}
				<span class="text-sm text-gray-400 group-hover:text-gray-300">
					Drop <span class="text-blue-400 font-mono">.osz</span> or
					<span class="text-blue-400 font-mono">.mug</span>
				</span>
			{:else}
				...
				{#if isConverting}
					<span class="text-xs text-blue-300 animate-pulse"
						>{conversionProgress || 'Processing...'}</span
					>
