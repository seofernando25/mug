...
{#if $isPaused}
	<div
		class="fixed inset-0 bg-black bg-opacity-75 flex flex-col items-center justify-center z-50 font-mono p-4"
		onclick={resumeGame}
		onkeydown={(e) => {
	...
	>
		<div class="bg-gray-800 p-8 rounded-xl shadow-2xl w-full max-w-md space-y-6">
			...
			<section class="space-y-4">
				<h3 class="text-xl font-semibold text-purple-300 border-b border-gray-700 pb-2">
					Audio Settings
				</h3>
				<div class="space-y-3">
					<div>
						<label for="pauseMasterVolume" class="block text-lg mb-1"
					...
					<div>
						<label for="pauseMusicVolume" class="block text-lg mb-1"
