	...
	{#if showSearch}
		...
		<div class="flex items-center justify-end px-8 py-4 gap-4">
			<div class="flex items-end space-x-1 mr-8">
				<button
					class="px-6 py-2 bg-pink-600 text-white font-bold rounded-t-lg transform hover:-translate-y-1 transition shadow-lg shadow-pink-500/20"
				>
					All Songs
				</button>
			</div>
			...
			<div class="relative group w-96">
				<div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
					<svg
						class="h-5 w-5 text-gray-400 group-focus-within:text-pink-500 transition"
						fill="none"
						viewBox="0 0 24 24"
						stroke="currentColor"
					>
			...
			{#each filteredSongs as song (song.id)}
				...
				<button
					...
					class="relative w-[85%] transition-all duration-200 ease-out group text-left
						   {isSelected
						? 'translate-x-[-20px] scale-105 z-20'
						: 'hover:translate-x-[-10px] opacity-80 hover:opacity-100'}"
					in:fly={{ x: 50, duration: 300, delay: 50 }}
				>
					<div
						class="h-24 w-full flex overflow-hidden rounded-l-xl border-r-4 shadow-xl backdrop-blur-md
							  {isSelected
							? 'bg-gray-800/90 border-pink-500 ring-1 ring-pink-500/50'
							: 'bg-gray-900/60 border-blue-500/50 hover:border-blue-400'}"
					>
						<div
							class="w-32 h-full bg-cover bg-center shrink-0"
						...
						<div class="flex-1 p-3 flex flex-col justify-center relative overflow-hidden">
							<span
								class="absolute right-2 top-0 text-2xl font-black text-white/5 pointer-events-none italic select-none uppercase tracking-wider"
							>
							...
							<div class="mt-1 flex items-center gap-2">
								{#if isSelected}
									<span
										class="text-pink-400 text-xs font-bold uppercase tracking-wider animate-pulse"
									>
										Click to play
									</span>
								{/if}
								{#if song.difficulties && song.difficulties.length > 0}
									<span class="text-xs text-gray-400">
										...
										{song.difficulties.length === 1 ? 'difficulty' : 'difficulties'}
									</span>
								{/if}
							</div>
						</div>
