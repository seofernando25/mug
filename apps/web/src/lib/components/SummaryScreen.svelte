<script lang="ts">
	...
	const leaderboard = $derived(Object.values($matchState).sort((a, b) => b.score - a.score));
	...
	>
		...
		<div class="px-10 py-8 grid grid-cols-1 md:grid-cols-2 gap-8">
			...
			<div class="space-y-1">
				<span class="text-[10px] font-black uppercase tracking-widest text-gray-500"
					>YOUR TOTAL SCORE</span
				>
				<div
					class="text-5xl font-black italic tracking-tighter text-white tabular-nums drop-shadow-[0_0_15px_rgba(255,255,255,0.2)]"
				>
					{formattedScore}
				</div>
			</div>
			...
			<div class="space-y-1">
				...
				>
				<div
					class="text-5xl font-black italic tracking-tighter text-pink-500 tabular-nums drop-shadow-[0_0_15px_rgba(236,72,153,0.3)]"
				>
		...
		{#if isMultiplayer && leaderboard.length > 0}
			<div class="px-10 pb-10 flex flex-col gap-2">
				<span class="text-[10px] font-black uppercase tracking-widest text-gray-500 mb-2"
					>FINAL RANKING</span
				>
				<div class="flex flex-col gap-1 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
					{#each leaderboard as peer, i (peer.userId)}
						<div
							class="flex items-center justify-between p-3 rounded-xl border transition-all {peer.finished
								? 'border-cyan-500/30 bg-cyan-500/5'
								: 'border-white/5 bg-white/5 opacity-50'}"
						>
							<div class="flex items-center gap-4">
								<span class="text-xs font-black italic text-gray-500 tabular-nums w-4"
									>#{i + 1}</span
								>
								<div class="flex flex-col">
									...
									{#if !peer.finished}
										<span
											class="text-[8px] font-bold text-cyan-400 uppercase tracking-widest mt-1 animate-pulse"
											>PLAYING...</span
										>
									{/if}
								</div>
