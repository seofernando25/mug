<script lang="ts">
	...
	// Derived
	const activeSong = $derived(songs.find((s: SongWheelItem) => s.id === selectedId) || songs[0]);
...
{#if isOpen}
	...
	>
		...
		<div
			class="relative z-10 w-[40%] h-full flex flex-col p-12 pt-24 space-y-8"
			in:fly={{ x: -50, duration: 400 }}
		>
			<div>
				<h1
					class="text-5xl font-black tracking-tight italic text-white drop-shadow-lg leading-tight"
				>
					{activeSong?.title}
				</h1>
								...
								{#each activeSong.difficulties as difficulty}
									...
									>
										...
										{#if selectedDifficulty === difficulty}
											<span class="text-xs bg-white/20 px-2 py-0.5 rounded text-white"
												>SELECTED</span
											>
										{/if}
									</button>
			...
			<button
				onclick={onClose}
				class="self-start text-gray-500 hover:text-white transition flex items-center gap-2"
			>
				<span>← BACK</span>
			</button>
		</div>
