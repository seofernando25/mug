// src/lib/server/conversion/index.ts
// This file runs on the SERVER side.
// It should NOT import database or S3 client.

import JSZip from 'jszip'; // JSZip is used server-side now

// Define expected structures for clarity (optional, but good practice)
...
function parseOsuFileContent(content: string): ParsedOsuData {
	const sections: Record<string, string[]> = {}; // Use Record<string, string[]>
	let currentSection = '';
	...
	if (fileExtension === 'osz') {
		...
		if (osuDataWithEvents) {
			...
			if (bgEvent) {
				...
				if (parts.length > 2) {
					let bgFilename = parts[2].replace(/"/g, ''); // Filename is usually in quotes
