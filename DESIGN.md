...
- **Primary Button:** Solid vibrant color (Purple/Pink gradient), rounded corners (`rounded`, `rounded-lg`, or `rounded-xl` for big buttons), shadow effects.
  - _Hover:_ `hover:scale-105`, brightness increase.
...
<h1 class="text-9xl font-black italic tracking-tighter text-white drop-shadow-2xl">
	MUG<span class="text-transparent bg-clip-text bg-linear-to-br from-purple-400 to-cyan-400"
		>.</span
	>
</h1>
```
...
```html
<button
	class="px-6 py-3 rounded-lg font-bold bg-purple-400 text-white hover:bg-purple-500 transition"
>
	ACTION
</button>
```
